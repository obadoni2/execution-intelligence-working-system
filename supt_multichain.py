...
def _mean(values: List[float], default: float = 0.0) -> float:
    ...
    return float(sum(clean) / len(clean)) if clean else default
...
def fetch_evm_block_window(rpc_url: str, chain: str, window_size: int = DEFAULT_WINDOW) -> ChainState:
    latest = _hex_to_int(_rpc_call(rpc_url, "eth_blockNumber"))
    ...
    return ChainState(
        ...
        metric_value=round(gas_ratios[-1], 6) if gas_ratios else 0.0,
        d_ij=d_ij,
...
def fetch_bitcoin_state(window_size: int = DEFAULT_WINDOW) -> ChainState:
    ...
    return ChainState(
        ...
        metric_value=round(tx_counts[-1], 6) if tx_counts else 0.0,
        d_ij=d_ij,
        regime=classify_regime(d_ij),
...
def fetch_solana_state(window_size: int = DEFAULT_WINDOW) -> ChainState:
    ...
    return ChainState(
        ...
        height=slot,
        metric_name="tps",
        metric_value=round(tps_values[-1], 6) if tps_values else 0.0,
        d_ij=d_ij,
        regime=classify_regime(d_ij),
...
def run_snapshot(window_size: int = DEFAULT_WINDOW) -> MultiChainSnapshot:
    ethereum = safe_probe("ethereum", lambda: fetch_evm_block_window(ETH_RPC_URL, "ethereum", window_size))
    base = safe_probe("base", lambda: fetch_evm_block_window(BASE_RPC_URL, "base", window_size))
    bitcoin = safe_probe("bitcoin", lambda: fetch_bitcoin_state(window_size))
    solana = safe_probe("solana", lambda: fetch_solana_state(window_size))

    divergence = compute_cross_chain_divergence([ethereum, base, bitcoin, solana])

    return MultiChainSnapshot(
        timestamp=_now_iso(),
        ethereum=ethereum,
        base=base,
        bitcoin=bitcoin,
...
def main() -> None:
    ...
    snapshot = run_snapshot(DEFAULT_WINDOW)
